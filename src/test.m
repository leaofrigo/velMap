function p = test(xxCentro,NumOfCellsX,cellDepths,cellDepthsCentro,NumOfCellsY,VelVector,...
...
switch Direcao
    case 'North'
        ...
        savefig(gcf,[Directory 'Session ' OpenFiles '\Velocity_North_Smooth.fig']);
    case 'East'
        title('Velocity East');
        [LowerVE,UpperVE] = limites(VelVector); 
        caxis([LowerVE UpperVE]);
        savefig(gcf,[Directory 'Session ' OpenFiles '/Velocity_East_Smooth.fig']);
    case 'Upstream'
        title('Velocity Upstream');
        [LowerVU,UpperVU] = limites(VelVector); 
        caxis([LowerVU UpperVU]);
        savefig(gcf,[Directory 'Session ' OpenFiles '/Velocity_Upstream_Smooth.fig']);
    case 'Error'
    ...
    case 'Magnitude'
        ...
        hold on
        savefig(gcf,[Directory 'Session ' OpenFiles '/Velocity_Magnitude_Smooth.fig']);
    case 'Transverse'
        [LowerVR,UpperVR] = limites(VelVector); 
        caxis([LowerVR UpperVR]);
        title('Velocity transverse')
        savefig(gcf,[Directory 'Session ' OpenFiles '/Velocity_Transversal_Smooth.fig']);
    case 'Longitudinal'
        title('Velocity Longitudinal')
        [LowerVL,UpperVL] = limites(VelVector); 
        caxis([LowerVL UpperVL]);
        savefig(gcf,[Directory 'Session ' OpenFiles '/Velocity_Longitudinal_Smooth.fig']);
end
